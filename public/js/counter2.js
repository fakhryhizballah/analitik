...
async function views(){
    let host = getHostname.href;
    const cookieExists = document.cookie.split(';').some(item => item.trim().startsWith(`${host}=`));
    if (!cookieExists) {
        let result = await fetch('https://count.spairum.my.id/api/track/view', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        console.log(result);
        document.cookie = `${host}=${result.views}; path=/; max-age=60;`;
    }
